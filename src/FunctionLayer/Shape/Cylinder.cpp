...
bool Cylinder::rayIntersectShape(Ray &ray, int *primID, float *u,
  ...
  if (check_intersect(t0))
    return true;
  else
    return check_intersect(t1);
}
...
void Cylinder::fillIntersection(float distance, int primID, float u, float v,
  ...
  Point3f localSpacePosition{radius * std::cos(phi), radius * std::sin(phi),
                             pz};
  Vector3f localSpaceNormal{radius * std::cos(phi), radius * std::sin(phi),
                            0.f};
